...
class SegmentAnythingSegmenter:
    def __init__(self, config, device="cuda"):
        self.device = device
        self.sam2_checkpoint = "/home/huteng/lliqirui/RealWonder/submodules/sam2/checkpoints/sam2.1_hiera_large.pt"
    ...
    def __call__(self, image):
        return self.predict_masks(
            image,
            self.config['all_object_points'],
            self.config['all_object_masks_idx'],
    ...
    def predict_masks(
        ...
        if isinstance(all_object_masks_idx, (list, tuple)):
            all_object_masks_idx = list(all_object_masks_idx)
        elif hasattr(all_object_masks_idx, '__iter__') and not isinstance(all_object_masks_idx, (str, bytes)):
            all_object_masks_idx = list(all_object_masks_idx)
        else:
        ...
        for object_idx, object_points in enumerate(all_object_points):
            debug_object_idx = object_idx + int(debug_index_offset)
            # for each object
            object_points = np.array(object_points)
            # Config points are already original-image pixel coordinates.
            ...
            plt.axis('on')
            plt.savefig(f"debug/sam2/{input_prefix}_{debug_object_idx:02d}.png")
            plt.close()
            ...
            show_masks(image, masks, scores, f"{mask_prefix}_{debug_object_idx:02d}", point_coords=object_points_xy, input_labels=object_point_labels, borders=True)
            try:
                mask_idx = int(all_object_masks_idx[object_idx])
            except (IndexError, TypeError, ValueError):
                mask_idx = 0
            mask_idx = max(0, min(mask_idx, len(masks) - 1))
            output_masks.append(masks[mask_idx])
