...
def resize_and_crop_pil(image: Image.Image, start_y=None) -> Image.Image:
    ...
    if width == 512 and height == 512:
        # 如果是 512x512，执行原逻辑：先 resize 到 832x832，再裁剪成 832x480
        ...
        final_image = resized_image.crop((start_x, start_y, start_x + crop_width, start_y + crop_height))
    else:
        # 如果是其他尺寸，直接 resize 到 832x480
        final_image = image.resize((832, 480), resample=Image.BILINEAR)
...
def save_mask_kps(mask, kps_h, kps_w, save_path):
    ...
    overlay = np.repeat(mask_img[..., None], 3, axis=-1)
    radius = max(2, int(round(min(mask_img.shape[:2]) * 0.006)))
    for h, w in zip(kps_h.cpu().numpy(), kps_w.cpu().numpy()):
        cv2.circle(overlay, (int(w), int(h)), radius, (255, 0, 0), thickness=-1)
...
def visualize_optical_flow_advanced(frames_folder, flows_npy_path, output_folder, 
    ...
    frame_files = sorted(glob(os.path.join(frames_folder, "*.png")) + 
...
def render_mesh_with_occlusion_detection(vertices, faces, colors, camera, 
    ...
    raster_settings = RasterizationSettings(
        image_size=image_size, # 接受 (H, W)
        blur_radius=0.0,
        faces_per_pixel=1,
        bin_size=0,            # 核心修改：改为 0，使用朴素光栅化，消除警告
...
def detect_occluded_vertices(vertices, camera, depth_map, image_size, depth_tolerance, device):
    ...
    # 核心修改：分别对 X 和 Y 坐标按对应的宽高进行 clamp
    pixel_coords[:, 0] = torch.clamp(pixel_coords[:, 0], 0, W - 1)
    pixel_coords[:, 1] = torch.clamp(pixel_coords[:, 1], 0, H - 1)
    
    # 核心修改：分别根据 W 和 H 进行反转计算
    pixel_x = W - 1 - pixel_coords[:, 0].long()
    pixel_y = H - 1 - pixel_coords[:, 1].long()
